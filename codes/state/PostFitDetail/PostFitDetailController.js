(function(angular) {
  ...
  PostFitDetailController.$inject = [
    ...
    'PostFitDetailModel', 'Post', 'Comment', 'Message', 'U', 'Preload'
  ];
  ...
  function PostFitDetailController(
    ...
    PostFitDetail.refresh = refresh;
    PostFitDetail.loadMoreComments = loadMoreComments;
    PostFitDetail.destroyPost = destroyPost;
    PostFitDetail.createComment = createComment;
    PostFitDetail.destroyComment = destroyComment;

    // AppSpecific
    PostFitDetail.showBubble = false;
      ...
      if (!U.hasPreviousStates(noLoadingStates)) {
        return initPromise
          .then(function(array) {
            ...
            U.bindData(post, PostFitDetailModel, 'post');
            U.bindData(commentsWrapper, PostFitDetailModel, 'comments');
          })
          .catch(function(err) {
            U.error(err);
          });
    ...
    function loadMoreComments() {
      var last = PostFitDetailModel.comments.length - 1;
      return findComments({
          id: {
            '<': PostFitDetailModel.comments[last].id
          }
        })
        .then(function(commentsWrapper) {
          U.appendData(commentsWrapper, PostFitDetailModel, 'comments');
        })
        .catch(function(err) {
          U.error(err);
        })
